...
| **figma-console MCP** (`figma_export_tokens` / `figma_execute`) | ✅ fallback      | Needs the Desktop Bridge plugin connected. Gotcha: two concurrent Codex sessions each spawn a WS server (ports 9223+) and the bridge grabs the lowest — drive the sync from the session that owns the port. |
