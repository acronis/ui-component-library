...
under `.Codex/skills/figma-to-design-tokens/`. The key discipline: you **review
...
- **Use last snapshot** — work from `.Codex/skills/figma-to-design-tokens/snapshot/` files already present
...
- **Running**: report the connected file name, then run `ls -l .Codex/skills/figma-to-design-tokens/snapshot/` and show the timestamps of any existing snapshot files, then proceed to Phase 0.
...
Verify `.Codex/skills/figma-to-design-tokens/snapshot/` exists and contains
...
```
outputPath: ./.Codex/skills/figma-to-design-tokens/snapshot/
format: dtcg
```
...
```bash
mv .Codex/skills/figma-to-design-tokens/snapshot/tokens.tokens.json \
   .Codex/skills/figma-to-design-tokens/snapshot/variables.tokens.json
```

**1b. `figma_execute` → `styles.json`**
Pull **all** style types via the plugin API and save the raw result (envelope and
all) to `.Codex/skills/figma-to-design-tokens/snapshot/styles.json`.
...
Run the following code and save the raw result to `.Codex/skills/figma-to-design-tokens/snapshot/variables-meta.json`:
...
Output: `.Codex/skills/figma-to-design-tokens/snapshot/figma-snapshot.json`
...
Write a plan file at `.Codex/plans/<slug>.md` with:
