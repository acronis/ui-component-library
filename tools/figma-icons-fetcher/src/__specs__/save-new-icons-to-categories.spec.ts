...
describe('saveNewIconsToCategories', () => {
  ...
  it('should do nothing when categorizeByColor is false', async () => {
    ...
    const result = await saveNewIconsToCategories(
      config,
      downloadedIcons,
      new Set()
    );
  ...
  it('should add only NEW monocolor icons', async () => {
    await fs.writeFile(
      path.join(outputDir, 'new-icon.svg'),
      '<svg><path fill="#000"/></svg>'
    );
    await fs.writeFile(
      path.join(monoColorDir, 'existing-icon.svg'),
      '<svg></svg>'
    );

    const config = {
      categorizeByColor: true,
      outputDir,
      monoColorDir,
      multiColorDir,
    };
    ...
    const newIconExists = await fs
      .access(path.join(monoColorDir, 'new-icon.svg'))
      .then(() => true)
      .catch(() => false);
  ...
  it('should add only NEW multicolor icons', async () => {
    await fs.writeFile(
      path.join(outputDir, 'new-multi.svg'),
      '<svg><path fill="#000"/><path fill="#FFF"/></svg>'
    );
    ...
    const newIconExists = await fs
      .access(path.join(multiColorDir, 'new-multi.svg'))
      .then(() => true)
      .catch(() => false);
    expect(newIconExists).toBe(true);
  });
  ...
  it('should calculate total counts correctly', async () => {
    ...
    await fs.writeFile(
      path.join(multiColorDir, 'existing3.svg'),
      '<svg></svg>'
    );
  ...
  it('should copy icon content correctly', async () => {
    ...
    const copiedContent = await fs.readFile(
      path.join(monoColorDir, 'test-icon.svg'),
      'utf8'
    );
  ...
  it('should create directories if they do not exist', async () => {
    ...
    const monoExists = await fs
      .access(monoColorDir)
      .then(() => true)
      .catch(() => false);
    const multiExists = await fs
      .access(multiColorDir)
      .then(() => true)
      .catch(() => false);
