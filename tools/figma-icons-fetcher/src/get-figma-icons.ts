 ...
 */
async function fetchPagesByName(
  ...
  const structureResponse = await figmaClient.get<FileStructureResponse>(
    `/files/${config.fileKey}?depth=1`
  );

  if (structureResponse.data.err) {
    throw new Error(
      `Cannot get Figma file structure: ${structureResponse.data.err}`
    );
  }
  ...
  console.log(
    chalk.cyan(
      `Fetched structure in ${(structureEndTime - structureStartTime) / 1000}s`
    )
  );
  ...
  const targetPages = allPages.filter((page) =>
    config.pageNames.includes(page.name)
  );
  ...
  console.log(
    `Found ${targetPages.length} pages: ${targetPages.map((p) => p.name).join(', ')}`
  );
  ...
  const pagesResponse = await figmaClient.get<NodesResponse>(
    `/files/${config.fileKey}/nodes?ids=${pageIds}`
  );
  ...
  console.log(
    chalk.cyan(`Fetched pages in ${(pagesEndTime - pagesStartTime) / 1000}s\n`)
  );
...
async function fetchNode(
  ...
  const response = await figmaClient.get<NodesResponse>(
    `/files/${config.fileKey}/nodes?ids=${config.nodeId}`
  );

  if (response.data.err) {
    throw new Error(
      `Cannot get Figma node ${config.nodeId}: ${response.data.err}`
    );
  }

  const entry =
    response.data.nodes[config.nodeId!] ??
    Object.values(response.data.nodes)[0];
  if (!entry) {
    throw new Error(
      `Node ${config.nodeId} not found in file ${config.fileKey}`
    );
  }

  console.log(
    chalk.cyan(`Fetched node in ${(Date.now() - startTime) / 1000}s\n`)
  );

  return [
    { id: config.nodeId!, name: entry.document.name, document: entry.document },
  ];
}
...
export async function getFigmaIcons(
  ...
  if (!config.fileKey) {
    throw new Error(
      'File key not found. Please add FIGMA_FETCHER_FILE_KEY to .env.local'
    );
  }

  if (!config.nodeId && !config.pageNames.length) {
    throw new Error(
      'Provide FIGMA_FETCHER_NODE_ID or FIGMA_FETCHER_PAGE_NAMES to locate the icons.'
    );
  }
  ...
  if (!icons.length) {
    const source = config.nodeId
      ? `node ${config.nodeId}`
      : `pages: ${config.pageNames.join(', ')}`;
    throw new Error(
      `No icons found using the "${config.selectionStrategy}" strategy in ${source}`
    );
  }
