...
export function assertPackSchema(pack: PackManifest): void {
  ...
  if (!valid) {
    const detail = (validatePack.errors ?? [])
      ...
      .join('; ');
    throw new Error(
      `${pack.name}: manifest fails pack.schema.json — ${detail}`
    );
  }
}
...
export function resolveAsset(
  ...
  const merged: Record<string, VariantValue> = {
    ...pack.values,
    ...asset.values,
  };
  ...
  if (assetFlags.length > 1) {
    fail(
      ...
      id,
      null,
      `multiple canonical markers in asset values (${assetFlags.join(', ')})`
    );
  }
  if (packFlags.length !== 1) {
    fail(
      ...
      pack.name,
      null,
      `pack values must flag exactly one canonical (found ${packFlags.length})`
    );
  }
  ...
  if (canonicalVal === undefined) {
    fail(
      ...
      id,
      canonical,
      'effective canonical was opted out or is missing'
    );
  }
  if (!isSource(canonicalVal)) {
    fail(
      ...
      id,
      canonical,
      'effective canonical must resolve to a $file source (not computed/marker/null)'
    );
  }
  ...
  const resolveChain = (
    variant: string,
    seen: Set<string>
  ): { leafFile: string; leafExt: string; rules: Rule[] } => {
    if (seen.has(variant)) {
      fail(
        pack.name,
        id,
        variant,
        `derivation cycle: ${[...seen, variant].join(' → ')}`
      );
    }
    ...
    if (val === undefined) {
      fail(
        ...
        id,
        variant,
        'referenced variant is missing or opted out'
      );
    }
    if (isSource(val)) {
      ...
      if (!exists)
        fail(
          ...
          id,
          variant,
          `$file does not exist on disk: ${val.$file}`
        );
      ...
      if (!EXT_FOR_TYPE[type].includes(ext)) {
        fail(
          ...
          id,
          variant,
          `extension .${ext} is not valid for $type "${type}" (expected ${EXT_FOR_TYPE[type].join('/')})`
        );
      }
    ...
    if (isComputed(val)) {
      if (type === 'raster') {
        fail(
          ...
          id,
          variant,
          'raster assets cannot have computed variants (scale/stroke are vector ops)'
        );
      }
      ...
      const ruleObjs = val.$rules.map((rid) => {
        ...
        if (!r)
          fail(
            ...
            id,
            variant,
            `unknown rule id "${rid}" (no rules/${rid}.json)`
          );
        return r as Rule;
      });
      const sub = resolveChain(source, new Set(seen).add(variant));
      return {
        leafFile: sub.leafFile,
        leafExt: sub.leafExt,
        rules: [...sub.rules, ...ruleObjs],
      };
    }
    ...
    return fail(
      ...
      id,
      variant,
      'variant has no $file or $rules (bare canonical marker without a source)'
    ) as never;
  };
...
export function resolvePack(
  ...
  return Object.entries(pack.assets ?? {}).map(([id, asset]) =>
    resolveAsset(pack, id, asset, rules)
  );
}
