...
function normalizeToken(
  ...
  if (isObject(node['values'])) {
    if (mode in node['values']) {
      value = node['values'][mode];
    } else if (defaultMode && defaultMode in node['values']) {
      value = node['values'][defaultMode]; // fall back to default-brand value
    } else {
      return undefined; // token defines neither this mode nor the default
    }
  } else {
...
function walk(
  ...
  for (const [key, child] of Object.entries(node)) {
    ...
    const normalized = walk(child, type, mode, platformId, defaultMode);
 ...
 *
 * When `defaultMode` is provided and a token has no value for `mode`, its
 * `defaultMode` value is used instead of omitting the token. Pass the
 * default-brand name for the brand-axis tiers (semantics, components) so
 * that brand DTCG views include every token — component alias references
 * resolve correctly even when a brand has no explicit override.
 */
