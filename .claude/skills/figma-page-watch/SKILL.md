...
2. **Fallback — metadata fingerprint (always available).** With only the official
   ...
   ```bash
   # from a saved file…
   node .claude/skills/figma-page-watch/scripts/page-watch.mjs fingerprint /tmp/<page>.txt
...
- **REST (preferred, gets every page):** provide a Figma token and let the script
  ...
  ```bash
  node .claude/skills/figma-page-watch/scripts/page-watch.mjs sync --fetch
  ```

  The token comes from the env (`FIGMA_TOKEN` / `FIGMA_PAT` / `FIGMA_ACCESS_TOKEN`)
  **or** a gitignored `.env.local` next to the skill — copy
  `.claude/skills/figma-page-watch/.env.local.example` → `.env.local` and paste a
...
- **Manual / MCP fallback:** pipe a page list in (accepts JSON or the
  ...
  ```bash
  node .claude/skills/figma-page-watch/scripts/page-watch.mjs sync - <<'EOF'
  - 419:1493: Index
  - 5181:6800: Accordion
  …
  EOF
...
```bash
node .claude/skills/figma-page-watch/scripts/page-watch.mjs plan /tmp/current.json
```
...
```bash
node .claude/skills/figma-page-watch/scripts/page-watch.mjs record /tmp/current.json
