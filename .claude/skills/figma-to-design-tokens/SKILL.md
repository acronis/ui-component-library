...
- **Components** are emitted only for the `DEFAULT_COMPONENTS` allowlist in `tools/token-emit/helpers/emit-components-builder.mjs` (today, 32 names: `Avatar`, `Breadcrumb`, `Button`, `ButtonIcon`, `ButtonIconInput`, `ButtonMenu`, `CardFilter`, `Checkbox`, `Chip`, `Dialog`, `Footer`, `Icon`, `InputDatePicker`, `InputPassword`, `InputSearch`, `InputSelect`, `InputText`, `InputTextArea`, `Link`, `Loading`, `Menu`, `MenuItem`, `Popover`, `Radio`, `Resizable`, `SearchGlobal`, `SidebarPrimary`, `SidebarSecondary`, `Switch`, `Table`, `Tag`, `Tooltip`). Extend the allowlist to pull more.
  - Four of those (`Icon`, `Menu`, `MenuItem`, `SearchGlobal`) have **no matching group in the snapshot**, so 28 of the 32 actually emit. An allowlist entry with no Figma group is silently skipped, not an error — so the allowlist length and the emitted-tier count legitimately differ.
  - Groups that exist in the snapshot but are deliberately **not** allowlisted: `AlertRibbon`, `ButtonGroup`, `Carousel`, `Chat`, `InputOTP`, `Notification`, `SegmentControl`, `ToastAlert`.
