...
export class SemanticsEmitter {
  ...
  #emitColors(out, aliasErrors) {
    ...
    if (!colorsNode)
      throw new Error('Snapshot missing brand.semantics.colors subtree.');
    ...
    for (const { path, leaf } of this.#walkColorLeaves(colorsNode)) {
      ...
      for (const [figmaModeKey, modeRef] of Object.entries(lastSynced)) {
        ...
        if (
          modeRef !== null &&
          typeof modeRef === 'object' &&
          modeRef.colorSpace === 'hsl'
        ) {
          values[normalizeMode(figmaModeKey)] = {
            colorSpace: 'hsl',
            components: modeRef.components,
          };
          continue;
        }
        if (typeof modeRef !== 'string' || !modeRef.startsWith('{')) {
          aliasErrors.push(
            `${path.join('.')} mode ${figmaModeKey}: expected reference or hsl object, got ${JSON.stringify(modeRef)}`
          );
          continue;
        }
        ...
        try {
          ...
          if (!this.#aliasTranslator.validates(ourAlias)) {
            aliasErrors.push(
              `unknown palette target: ${ourAlias} (from ${path.join('.')} mode ${figmaModeKey})`
            );
          }
      ...
      if (leaf.$extensions?.['com.figma.hiddenFromPublishing'])
  ...
  *#walkColorLeaves(node, path = []) {
    ...
    if (
      '$value' in node ||
      ('$extensions' in node && node.$extensions?.['com.figma.variableId'])
    ) {
      ...
      for (const [key, leaf] of Object.entries(tokens)) {
        ...
        for (const [modeKey, modeVal] of Object.entries(modes)) {
          if (typeof modeVal !== 'string') {
            aliasErrors.push(
              `gradients.${group}.${key} mode ${modeKey}: expected CSS literal`
            );
            continue;
          }
          rawCss = modeVal;
  ...
  #parseCssGradient(css) {
    ...
    return parts.map((p) => {
      ...
      return {
        color: ColorUtils.hexToHslValue(sm[1]),
        position: ColorUtils.round(Number(sm[2]) / 100, 4),
      };
    });
  }
  ...
  #emitTypography(out) {
    ...
    for (const style of textStyles) {
      ...
      if (style.lineHeight?.unit !== 'PIXELS')
        throw new Error(
          `Text style "${style.name}" lineHeight unit ${style.lineHeight?.unit} — only PIXELS.`
        );
      if (
        ...
        throw new Error(
          `Text style "${style.name}" non-zero letterSpacing unit ${style.letterSpacing.unit} — only PIXELS.`
        );
      }
      ...
      const letterSpacing =
        style.letterSpacing.value === 0
          ? 0
          : ColorUtils.round(style.letterSpacing.value, 4);

      const composite = {
        fontFamily: this.#requireAlias(
          this.#typoMapper.fontFamily(family),
          'fontFamily',
          family,
          style.name
        ),
        fontSize: this.#aliasOrInline(
          this.#typoMapper.fontSize(fontSize),
          fontSize,
          'fontSize',
          style.name
        ),
        fontWeight: this.#requireAlias(
          this.#typoMapper.fontWeight(weight),
          'fontWeight',
          weight,
          style.name
        ),
        lineHeight: this.#aliasOrInline(
          this.#typoMapper.lineHeight(lineHeight),
          lineHeight,
          'lineHeight',
          style.name
        ),
        letterSpacing: this.#requireAlias(
          this.#typoMapper.letterSpacing(letterSpacing),
          'letterSpacing',
          letterSpacing,
          style.name
        ),
      ...
      if (style.textCase && style.textCase !== 'ORIGINAL')
        ext['com.acronis.textCase'] = style.textCase;
      if (style.textDecoration && style.textDecoration !== 'NONE')
        ext['com.acronis.textDecoration'] = style.textDecoration;
      ext['com.figma.styleId'] = style.id;

      TreeUtils.setPath(out.typography, ourPath, {
        $value: composite,
        platforms: ['PD'],
        $extensions: ext,
      });
  ...
  #requireAlias(alias, field, value, styleName) {
    ...
    throw new Error(
      `Text style "${styleName}": no primitive for ${field}=${JSON.stringify(value)} — extend primitives.json.`
    );
  }
  ...
  #aliasOrInline(alias, px, field, styleName) {
    if (alias !== null) return alias;
    console.warn(
      `Warning: "${styleName}" ${field} ${px}px has no matching primitive — inlined raw dimension.`
    );
