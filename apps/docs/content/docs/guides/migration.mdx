...
description: Migrate from @constructor-lab/shadcn-uikit to @constructor-lab/ui-react with component mapping and codemod recipes.
---
...
- `@constructor-lab/shadcn-uikit` (legacy)

To:

- `@constructor-lab/ui-react` (next generation)
...
```bash
pnpm remove @constructor-lab/shadcn-uikit
pnpm add @constructor-lab/ui-react
```

```tsx
// before
import '@constructor-lab/shadcn-uikit/styles';
...
Legacy code usually imports components from `@constructor-lab/shadcn-uikit/react`.
Move migrated components to `@constructor-lab/ui-react`.

```tsx
// before
import { Button, Input, Switch } from '@constructor-lab/shadcn-uikit/react';

// after
import { Button, Input, Switch } from '@constructor-lab/ui-react';
```
...
module.exports = function transformer(file, api) {
  ...
  const fromLegacy = new Set([
    '@constructor-lab/shadcn-uikit/react',
    '@constructor-lab/shadcn-uikit',
  ]);

  root.find(j.ImportDeclaration).forEach((path) => {
    if (!fromLegacy.has(path.node.source.value)) return;
    path.node.source = j.stringLiteral('@constructor-lab/ui-react');
  });
...
module.exports = function transformer(file, api) {
  ...
  root
    .find(j.ImportDeclaration, {
      source: { value: '@constructor-lab/ui-react' },
    })
    .forEach((path) => {
...
import { Dialog, Popover } from '@constructor-lab/shadcn-uikit/react';
```

- Re-test interactive components after migration (`Select`, `RadioGroup`, sidebars).
- Verify style import points to `@constructor-lab/ui-react/styles`.

## 6) Migrating icons

Icons migrate independently of components. Legacy
`@constructor-lab/shadcn-uikit` re-exports icons under UI Components library names
...
`@constructor-lab/icons-react/legacy-map` export (`legacy-icon-map.json`), a
committed static map. It maps every legacy icon name to its icons-react name per
pack variant, classifying each as:
...
npx tsc -b --noEmit && grep -R "@constructor-lab/shadcn-uikit" src
```
...
grep -R "@constructor-lab/shadcn-uikit" src

# ui-react usage
grep -R "@constructor-lab/ui-react" src
```
