...
description: How tree-shaking works in @spec-lab/ui-react and best practices for optimal bundle size.
---
...
`@spec-lab/ui-react` is built to be **tree-shakeable**, so your
...
`react`, `react-dom`, `@base-ui/react`, and `@spec-lab/icons-react` are
    ...
    rollupOptions: {
      output: {
        manualChunks: {
          ...
          'ui-components': ['@spec-lab/ui-react'],
        },
      },
    },
  },
...
import { Button } from '@spec-lab/ui-react/dist/components/ui/button/button';
```
...
import * as UI from '@spec-lab/ui-react';
```
...
const SidebarSecondary = lazy(() =>
  import('@spec-lab/ui-react').then((mod) => ({
...
const loadResizable = async () => {
  const { Resizable } = await import('@spec-lab/ui-react');
  return Resizable;
};
```
...
`@spec-lab/ui-react` is built for production:
