...
1. **Token model — keep what's shipped.** The canonical runtime model is
   ...
   `tools/style-dictionary` into `@constructor-lab/tokens`. We do **not**
...
- **Consumption:** `ui-react/src/styles/index.css` pulls the whole kit with a
  single `@import '@constructor-lab/tokens/css'` (primitives + semantics + every
...
`@constructor-lab/tokens/css` import. Gap is **ergonomics + contract**, not generation:

- Confirm/declare the single `@constructor-lab/tokens/css` **entry point** and the
  `[data-brand]` switch as public (see
  [versioning](../packages/tokens/context/versioning.md)).
- Document the consumption recipe in `ui-react`: one `@constructor-lab/tokens/css`
...
Import `@constructor-lab/tokens/css` once at the document; set `[data-brand]` for the
