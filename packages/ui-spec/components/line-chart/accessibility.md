...
- A line chart is inherently visual. **Pair it with a text alternative** — a
...
- Do **not** rely on color alone to distinguish series. Keep `showLegend` (or the
  tooltip) visible so each color is paired with a text label; the `lineStyle`
  (solid / dashed) and `curve` variants add a non-color channel between lines.
...
Chart chrome meets contrast in both themes via the semantic tokens. Line strokes
