...
- A radial-bar chart is inherently visual. **Pair it with a text alternative** —
  a caption, a summary sentence, or an adjacent data table carrying the same
...
- Keep `showLegend` on (or the tooltip visible) so each arc is named in text, not
  by color/position alone.
- The chrome (tooltip, legend) and the background track resolve to semantic
...
semantic tokens. Arc fills come from `config` (the `--ui-chart-*` palette by
