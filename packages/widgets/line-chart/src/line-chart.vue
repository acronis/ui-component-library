<template>
  ...
  >
    ...
    <template v-else>
        <div v-if="legend" class="el-line-chart__legend pb-8" ref="legend">
          <template v-if="$slots.legend">
            <slot name="legend" />
          </template>
        </div>
        <div
          class="el-line-chart__drawing"
          :style="{ height: height, width: renderWidth }"
        >
          <canvas ref="chartCanvas" @mouseleave="onCanvasLeave"/>
          <el-popover
            v-if="tooltip"
            ref="tooltipPopover"
            placement="top"
            trigger="manual"
            pointer="arrow"
            popper-class="el-line-chart__tooltip"
            :popper-options="ttPopperOptions"
          >
            <div class="el-text el-text--body-24 el-line-chart__tooltip-wrapper">
          ...
          <div
            v-if="tooltip"
            class="el-line-chart__tooltip-reference"
...
import { cleanData, isValidData, getDataMaxY } from '@/utils/widget';
...
export default {
  ...
  methods: {
    ...
    generateData() {
      this.area ? this.generateAreaChartDataset() : this.generateLineChartDataset();
    },
    ...
    renderChart() {
      ...
      const fontFamily = document.documentElement.style.getPropertyValue('--ui-kit-font');
    ...
    addLegendEvents(chart) {
      this.$nextTick(() => {
        ...
        const legends = this.$refs.legend.querySelectorAll('.el-line-chart__legend__item');
    ...
    generateLegend(chart) {
      ...
      datasets.forEach((item, index) => {
        ...
        li.className = 'el-line-chart__legend__item el-text el-text--body-24 mr-16';
        ...
        labelContainer.className = 'el-line-chart__legend__label';

        const label = document.createTextNode(item.label);
        labelContainer.appendChild(label);

        const separatorContainer = document.createElement('span');
        separatorContainer.className = 'el-line-chart__legend__separator mr-8';
        ...
        valueContainer.className = 'el-line-chart__legend__value';
        let value;
    ...
    formatValue(value) {
      const label = this.yPercent
        ...
        : this.$options.filters.appendUnit(value, this.parsedDataType, this.precision);
    ...
    renderTooltip(context) {
      ...
      if (tooltip.body) {
        ...
        tooltipTitle.className = 'el-text el-text--strong el-line-chart__tooltip-title';
        const text = document.createTextNode(titleLine);
        tooltipTitle.appendChild(text);
        const tooltipContent = document.createElement('div');
        tooltipContent.className = 'el-line-chart__tooltip-content';
        tooltip.dataPoints.forEach((point, index) => {
          ...
          if (!this.variableThreshold) {
            ...
            tooltipBullet.className = 'el-line-chart__bullet';
          ...
          tooltipLabel.className = 'el-line-chart__tooltip-label';
        ...
        const tooltipRoot = popover.popperElm.querySelector('.el-line-chart__tooltip-wrapper');
        ...
        if (!this.$slots.tooltipbody) {
          ...
          if (tooltipRoot.firstChild && tooltipRoot.firstChild.classList.contains('el-line-chart__tooltip-title')) {
    ...
    onCanvasClick(event) {
      ...
      if (this.chart) {
        ...
        this.$emit('timeline-click', time, event);
      }
    },
    addThresholdAnnotation({ chart }) {
      ...
      if (!dataToProcess.length || !ctx) return;
      return dataToProcess
        .map((data) => {
          let text = this.yPercent
          ? this.getPercent(data.threshold, this.data?.maxY || 100)
          : this.$options.filters.appendUnit(data.threshold, this.parsedDataType, this.precision);
