...
export const PROVIDED_PROPS = '__acv-provided-props';
...
export function configProps<T>(props: MaybeRef<T>, app?: App) {
  if (app) {
    ...
    const providedProps = computed(() => {
      ...
      return merge(upstreamProps.value, unref(props) as any);
    });
...
function toWarnPrefix(name: string) {
  return `[ui-kit:${name.charAt(0).toLocaleUpperCase() + name.substring(1)}]`;
}
...
export type UikitProps<T> = {
  [P in keyof T]: T[P] extends PropType<infer I>
    ? PropType<I & NonNullable<unknown>>
...
export function buildProps<T extends ComponentObjectPropsOptions>(props: T) {
  ...
  return Object.freeze({ ...common, ...props }) as Expand<UikitProps<typeof common & T>>;
}
